		...
		computed : {
			limitPagination(){
				if( this.corporate_pagination.last_page ){
					...
					return arr.slice(this.startIndex(), this.startIndex() + this.pagesToDisplay);
				}else{
			...
			searchedExportProperties(){
				...
				if( this.searchPropertiesText != '' ){
					return this.export_data_header.filter((value) => {
						// console.log( value );
						return (value.toLowerCase()).startsWith( this.searchPropertiesText.toLowerCase() );
					})
				}else{
		...
		methods: {
			...
			startIndex() {
				if (this.page_active > ((this.pagesToDisplay / 2) + 1)) {
					if ((this.page_active + Math.floor(this.pagesToDisplay / 2)) > this.corporate_pagination.last_page) {
